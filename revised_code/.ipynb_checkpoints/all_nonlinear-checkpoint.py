...
def gen_XY_nonlinear(n=1000, seed=0):
    """
    ...
    # Compute nonlinear transformations of X to generate potential outcomes
    M = np.dot(X, coef)
    Y0 = np.log(M**2 + 1)  # Nonlinear base outcome
    Y1 = Y0 + np.sign(np.dot(X, coef2))  # Treatment effect shifts the outcome
...
def get_data_nonlinear(n=1000, seed=42):
    """
    ...
    X, Y, coef2 = gen_XY_nonlinear(n=n, seed=seed)
